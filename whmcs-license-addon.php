...
// ── Fetch live BDT/USD rate from WHMCS currency feed ──
// WHMCS currencies.php returns: currency_code|rate|prefix|suffix|...
function getWHMCSRate($whmcs, $from = 'USD', $to = 'BDT') {
    $cache = sys_get_temp_dir() . '/rn_rate_' . $from . '_' . $to . '.txt';
    if (file_exists($cache) && (time() - filemtime($cache)) < 3600) { // 1hr cache
        $v = trim(file_get_contents($cache));
        if ($v && is_numeric($v)) return (float)$v;
    }
    $ch = curl_init($whmcs . '/feeds/currencies.php');
    ...
    $rate = null;
    if ($raw) {
        // Format: currencyid,code,prefix,suffix,rate,default
        foreach (explode("\n", trim($raw)) as $line) {
            $parts = preg_split('/[,;|]/', $line);
            // Try to find BDT row
            foreach ($parts as $pi => $part) {
                if (strtoupper(trim($part)) === strtoupper($to)) {
                    // Rate is usually in position 4 or 3
                    foreach ($parts as $rp) {
                        $rp = trim($rp);
                        if (is_numeric($rp) && (float)$rp > 1 && (float)$rp < 99999) {
                            $rate = (float)$rp; break;
                        }
                    }
                    break 2;
    ...
    // Fallback: 130
    if (!$rate || $rate <= 0) $rate = 130.0;
    file_put_contents($cache, $rate);
    return $rate;
}

$usd_to_bdt = getWHMCSRate($whmcs, 'USD', 'BDT');
foreach ($plans as &$p) {
    $p['name']          = whmcsFetch($whmcs, $p['pid'], 'name') ?: $p['name_fb'];
    $raw_price          = whmcsFetch($whmcs, $p['pid'], 'price', 'monthly', 1);
    $fb_prices = [2=>'25.00', 3=>'17.00', 1=>'40.00'];
    $p['price_monthly'] = $raw_price ?: '$' . ($fb_prices[$p['pid']] ?? '25.00');
    $p['price_num']     = (float) preg_replace('/[^0-9.]/', '', $p['price_monthly']);
    $p['price_bdt']     = round($p['price_num'] * $usd_to_bdt); // synced from WHMCS currency rate
    $p['price_yearly']  = round($p['price_num'] * 12 * 0.85); // 15% annual discount
}
unset($p);
...
// Schema FAQs
$schema_faqs = array_map(fn($f) => ['@type'=>'Question','name'=>$f['q'],'acceptedAnswer'=>['@type'=>'Answer','text'=>$f['a']]], $faqs);
?>
<!doctype html>
...
<div class="container">
    ...
    <div class="curr-page-bar">
        ...
        <span class="curr-rate-info" id="currRateInfo">1 USD = ৳<?=number_format($usd_to_bdt,2)?> (WHMCS rate)</span>
    </div>
    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="faq-wrap">
                ...
                <div class="faq-set">
                    <a <?=$i===0?'class="active"':''?> href="javascript:void(0)">
                        <?=htmlspecialchars($f['q'])?><i class="fas fa-angle-<?=$i===0?'down':'up'?>"></i>
                    </a>
        ...
        <div class="col-lg-6">
            <div class="faq-wrap">
                ...
                <div class="faq-set">
                    <a href="javascript:void(0)"><?=htmlspecialchars($f['q'])?><i class="fas fa-angle-up"></i></a>
                    <div class="content"><p><?=htmlspecialchars($f['a'])?></p></div>
                </div>
                <?php endforeach;?>
            </div>
        </div>
...
function applyConversion(curr, rate) {
    ...
    if(info) info.textContent = curr==='USD' ? 'Base currency' : '1 USD ≈ '+rate.toFixed(2)+' '+curr+' (WHMCS rate)';
...
// Fixed rates (USD base) — BDT synced from WHMCS currencies feed
var fixedRates = {USD:1, BDT:<?=number_format($usd_to_bdt,4,'.','')?>, EUR:0.92, GBP:0.79, INR:83.5, SAR:3.75, AED:3.67, SGD:1.34};
rates = fixedRates;
ratesFetched = true;

function fetchRates(curr) {
    applyConversion(curr, fixedRates[curr] || 1);
}
